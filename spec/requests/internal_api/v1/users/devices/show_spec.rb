...
RSpec.describe "Devices#show", type: :request do
  ...
  context "when logged in user is Owner" do
    ...
    context "when owner wants to check his own details" do
      before do
        send_request :get, internal_api_v1_user_device_path(user, device_of_user), headers: headers(user)
      end
    ...
    context "when user wants to see record of an employee of his own workspace" do
      before do
        ...
        send_request :get, internal_api_v1_user_device_path(employee, device_of_employee), headers: headers(user)
      end
    ...
    context "when user wants to check record of an employee of a different workspace" do
      before do
        ...
        send_request :get, internal_api_v1_user_device_path(user, employee), headers: headers(user)
      end
    ...
    context "when user sends request for user id which doesnt exist" do
      before do
        send_request :get, internal_api_v1_user_device_path(user, "abc"), headers: headers(user)
      end
