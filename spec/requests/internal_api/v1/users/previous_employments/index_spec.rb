    ...
    context "when user is owner" do
      context "when owner wants to check his own details" do
        before do
          ...
          sign_in user
          send_request :get, internal_api_v1_user_previous_employments_path(user), headers: auth_headers(user)
        end
      ...
      context "when owner and wants to check record of an employee" do
        context "when owner/admin wants to see record of an employee of his own workspace" do
          before do
            ...
            user.add_role :owner, company
            sign_in user
            send_request :get, internal_api_v1_user_previous_employments_path(employee), headers: auth_headers(user)
          end
        ...
        context "when owner/admin wants to check record of an employee of a different workspace" do
          before do
            ...
            user.add_role :owner, company
            sign_in user
            send_request :get, internal_api_v1_user_previous_employments_path(employee2), headers: auth_headers(user)
          end
        ...
        context "when owner sends request for user id which doesnt exist" do
          before do
            user.add_role :owner, company
            sign_in user
            send_request :get, internal_api_v1_user_previous_employments_path("abc"), headers: auth_headers(user)
          end
