  ...
  describe "POST #create" do
    context "when user is not confirmed" do
      ...
      it "sent confirmation instructions" do
        post internal_api_v1_users_resend_confirmation_email_path({ user: { email: user.email } })
    ...
    context "when user doesn't exist" do
      it "redirects to root_path" do
        post internal_api_v1_users_resend_confirmation_email_path({ user: { email: "miru@example.com" } })
