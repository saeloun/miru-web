...
RSpec.describe "InternalApi::V1::Team#destroy", type: :request do
  ...
  describe "when current user is admin" do
    ...
    context "when team member is present in company" do
      ...
      it "Discards the employments" do
        send_request :delete, internal_api_v1_team_path(@team_company_user.user), headers: auth_headers(admin_user)
    ...
    context "when team member is not present in company" do
      ...
      it "returns unsuccessful response with not_found status" do
        send_request :delete, internal_api_v1_team_path(@non_team_company_user.user), headers: auth_headers(admin_user)
    ...
    context "when team member is not discarded" do
      ...
      it "returns unsuccessful response with internal_server_error status" do
        send_request :delete, internal_api_v1_team_path(invalid_user), headers: auth_headers(admin_user)
    ...
    context "when team member is present in multiple company" do
      ...
      it "discard team member from only current company" do
        expect { send_request :delete, internal_api_v1_team_path(team_user), headers: auth_headers(admin_user) }
      ...
      it "updates the current_workspace_id of the user" do
        send_request :delete, internal_api_v1_team_path(team_user), headers: auth_headers(admin_user)
  ...
  context "when current user is an employee" do
    ...
    before do
      ...
      send_request :delete, internal_api_v1_team_path(team_user), headers: auth_headers(employee_user)
    end
