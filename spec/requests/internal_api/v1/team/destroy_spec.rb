...
RSpec.describe "InternalApi::V1::Team#destroy", type: :request do
  ...
  describe "when current user is admin" do
    ...
    context "when team member is not present in company" do
      ...
      before do
        @non_team_company_user = create(:company_user, user: team_user)
        create(:company_user, company:, user: admin_user)
        admin_user.add_role :admin, company
        sign_in admin_user
      end
    ...
    context "when team member is present in multiple company" do
      ...
      before do
        @team_company_user = create(:company_user, company:, user: team_user)
        create(:company_user, company: other_company_1, user: team_user)
        create(:company_user, company: other_company_2, user: team_user)
        admin_user.add_role :admin, company
        sign_in admin_user
      end
