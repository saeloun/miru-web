...
RSpec.describe "InternalApi::V1::Reports::TimeEntryController#download", type: :request do
  ...
  context "when user is an admin or owner" do
    ...
    context "when CSV file requested" do
      ...
      it "returns CSV data in response" do
        send_request :get, "/internal_api/v1/reports/time_entries/download.#{type}", headers: headers(admin)
  ...
  context "when user is an employee" do
    ...
    it "returns 403 status" do
      send_request :get, "/internal_api/v1/reports/time_entries/download.#{type}", headers: headers(employee)
  ...
  context "when user is a book keeper" do
    ...
    it "returns 403 status" do
      send_request :get, "/internal_api/v1/reports/time_entries/download.#{type}", headers: headers(book_keeper)
      expect(response).to have_http_status(:forbidden)
      expect(json_response["errors"]).to eq "You are not authorized to perform this action."
    end
  end
