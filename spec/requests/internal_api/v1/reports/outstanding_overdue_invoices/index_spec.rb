  ...
  context "when user is an admin" do
    ...
    context "when outstanding and overdue invoices reports page's request is made" do
      ...
      it "returns the clients data in alaphabetical order with invoices details" do
        expect(json_response["clients"][0].except("invoices")).to
        eq(JSON.parse(@expected_clients.first.except(:invoices).to_json))
        expect(json_response["clients"][0].slice("invoices")).to
        include(JSON.parse(@expected_clients.first.slice(:invoices).to_json))
        expect(json_response["clients"][0].slice("invoices")["invoices"].count).to
        eq(@expected_clients.first.slice(:invoices)[:invoices].count)

        expect(json_response["clients"][1].except("invoices")).to
        eq(JSON.parse(@expected_clients.last.except(:invoices).to_json))
        expect(json_response["clients"][1].slice("invoices")).to
        include(JSON.parse(@expected_clients.last.slice(:invoices).to_json))
        expect(json_response["clients"][1].slice("invoices")["invoices"].count).to
        eq(@expected_clients.last.slice(:invoices)[:invoices].count)
      end
