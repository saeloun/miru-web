...
RSpec.describe "InternalApi::V1::TimeoffEntries#destroy", type: :request do
  let(:company) { create(:company) }
  let(:user) { create(:user, current_workspace_id: company.id) }
  let(:leave) { create(:leave, company:) }
  let!(:leave_type) { create(:leave_type, leave:,allocation_period: "months", allocation_frequency: "per_quarter",
  ...
  context "when user is an admin" do
    ...
    describe "#destroy" do
      it "deletes timeoff entry successfully" do
        send_request :delete, internal_api_v1_timeoff_entry_path(id: timeoff_entry.id), headers: auth_headers(user)
