...
RSpec.describe "InternalApi::V1::TeamMembers::AvatarController#destroy", type: :request do
  ...
  context "when admin wants to remove avatar of employee of his company" do
    before do
      ...
      send_request :delete, internal_api_v1_team_avatar_path(user.id), headers: auth_headers(admin)
    end
  ...
  context "when owner wants to remove avatar of employee of his company" do
    before do
      ...
      user.add_role :employee, company
      sign_in owner
      send_request :delete, internal_api_v1_team_avatar_path(user.id), headers: auth_headers(owner)
    end
  ...
  context "when logged in admin wants to remove avatar of employee from a different company" do
    before do
      ...
      sign_in admin
      send_request :delete, internal_api_v1_team_avatar_path(user2.id), headers: auth_headers(admin)
    end
  ...
  context "when logged in owner wants to remove avatar of employee from a different company" do
    before do
      ...
      user2.add_role :employee, company2
      sign_in owner
      send_request :delete, internal_api_v1_team_avatar_path(user2.id), headers: auth_headers(owner)
    end
