...
RSpec.describe "PreviousEmployments#create", type: :request do
  ...
  context "when Owner wants to create Previous Employments details of employee of his company" do
    before do
      ...
      sign_in user
      send_request :post, internal_api_v1_team_previous_employments_path(
        team_id: employment.id,
        params: {
          previous_employments: @previous_employment_details
        }
      )
    end
    ...
    it "is successful" do
      ...
      expect(json_response["previous_employments"][0]["company_name"]).to eq(
        JSON.parse(@previous_employment_details[:company_name].to_json))
      expect(json_response["previous_employments"][0]["role"]).to eq(
        JSON.parse(@previous_employment_details[:role].to_json))
    end
  end
  ...
  context "when logged in user wants to create Previous Employments details of
  ...
  context "when logged in Owner wants to create Previous Employments details of
  ...
  context "when logged in Admin wants to create Previous Employments details of
    employee from a different company" do
    before do
      employment2 = create(:employment, user: user2, company: company2)
      user.add_role :admin, company
