  ...
  describe "GET edit" do
    context "when user is an admin" do
      ...
      it "returns 200 successful response with data" do
        send_request :get, edit_api_v1_invoice_path(invoice), headers: auth_headers(admin)
    ...
    context "when user is an employee" do
      ...
      it "returns 403 status with an error message" do
        send_request :get, edit_api_v1_invoice_path(invoice), headers: auth_headers(employee)
    ...
    context "when user is a book keeper" do
      ...
      it "returns 403 status with an error message" do
        send_request :get, edit_api_v1_invoice_path(invoice), headers: auth_headers(book_keeper)
        expect(response).to have_http_status(:forbidden)
        expect(json_response["errors"]).to eq "You are not authorized to perform this action."
      end
    end
