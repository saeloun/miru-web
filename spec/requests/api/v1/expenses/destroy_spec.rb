...
RSpec.describe "Api::V1::Expense#destroy", type: :request do
  ...
  context "when the user is an employee" do
    before do
      sign_in employee
      send_request :delete, api_v1_expense_path(expense), headers: auth_headers(employee)
    end

    it "is not permitted to delete an expense" do
  ...
  context "when the user is a book keeper" do
    before do
      sign_in book_keeper
      send_request :delete, api_v1_expense_path(expense), headers: auth_headers(book_keeper)
    end
  ...
  context "when unauthenticated" do
    it "is not permitted to delete an expense" do
      send_request :delete, api_v1_expense_path(expense)
