...
RSpec.describe "Api::V1::Expense#index", type: :request do
  ...
  context "when user is an admin" do
    ...
    describe "#index" do
      ...
      it "includes non-base-currency expenses converted to base currency in summary totals" do
        other_currency = company.base_currency == "USD" ? "INR" : "USD"
        foreign_expense = create(
          :expense, company:, user: admin, currency: other_currency, amount: 123.45, expense_type: :business
        )
        ...
        expect(summary["baseCurrency"]).to eq(company.base_currency)
        expect(BigDecimal(summary["totalAmount"].to_s))
          .to eq(expense1.amount + expense2.amount + foreign_expense.base_currency_amount)
      end
