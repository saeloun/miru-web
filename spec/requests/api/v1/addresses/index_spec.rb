...
RSpec.describe "Addresses#index", type: :request do
  ...
  context "when logged in user is an Owner" do
    ...
    context "when user wants to see addresses of his Company" do
      before do
        send_request :get, api_v1_company_addresses_path(company), headers: auth_headers(user)
      end
    ...
    context "when user wants to see address of an Employee of his company" do
      before do
        ...
        send_request :get, api_v1_user_addresses_path(employee), headers: auth_headers(user)
      end
    ...
    context "when user wants to see address of a Company other than his workspace" do
      before do
        send_request :get, api_v1_company_addresses_path(company2), headers: auth_headers(user)
      end
  ...
  context "when logged in user is an Employee" do
    ...
    context "when user wants to see his own Company's address detail" do
      before do
        send_request :get, api_v1_company_addresses_path(company), headers: auth_headers(employee)
      end
    ...
    context "when user wants to see his own address detail" do
      before do
        send_request :get, api_v1_user_addresses_path(employee), headers: auth_headers(employee)
      end
