...
RSpec.describe "Api::V1::Companies::PurgeLogo#destroy", type: :request do
  ...
  context "when user is not an admin" do
    before do
      sign_in user2
      send_request(:delete, "/internal_api/v1/companies/#{company.id}/purge_logo", headers: auth_headers(user2))
    end
  ...
  context "when user is an admin" do
    before do
      ...
      sign_in user1
      send_request(:delete, "/internal_api/v1/companies/#{company.id}/purge_logo", headers: auth_headers(user1))
    end
