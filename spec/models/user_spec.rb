...
RSpec.describe User, type: :model do
  ...
  describe "allowed clients" do
    let(:company2) { create(:company) }
    let(:client_1) { create(:client, company:, name: "Beta") }
    let(:client_2) { create(:client, company:) }
    let!(:client_3) { create(:client, company:, name: "Alpha") }
    let(:client_4) { create(:client, company: company2) }
    let(:project_1) { create(:project, client: client_1) }
    ...
    context "when user is an admin/owner" do
      ...
      it "returns the list of allowed clients" do
        expect(user.allowed_clients(company).pluck(:id)).to eq([client_3.id, client_1.id])
      end
    end
