  ...
  describe "User.search" do
    ...
    describe "pagination" do
      ...
      it "handles last page correctly" do
        ...
        full_pages = (total_users / 5)
        remaining_users = total_users % 5

        if remaining_users > 0
          last_page_number = full_pages + 1
          results = User.search("", page: last_page_number, per_page: 5)
          expect(results.count).to eq(remaining_users)
        else
          last_page_number = full_pages
          results = User.search("", page: last_page_number, per_page: 5)
          expect(results.count).to eq(5)
        end
      end
