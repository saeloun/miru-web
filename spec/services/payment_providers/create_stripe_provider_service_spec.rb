...
RSpec.describe PaymentProviders::CreateStripeProviderService do
  ...
  describe "#process" do
    ...
    context "when stripe account is not connected" do
      it "payment provide is created" do
        stripe_connected_account = StripeConnectedAccount.new
        allow(stripe_connected_account).to receive(:details_submitted).and_return(true)
        allow_any_instance_of(Company).to receive(:stripe_connected_account).and_return(nil)
        expect { described_class.new(company).process }.to change(PaymentsProvider, :count).by(0)
      end
    end
  end
end
