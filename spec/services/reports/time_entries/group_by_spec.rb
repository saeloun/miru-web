...
RSpec.describe "GroupBy", type: :service do
  describe "#is_valid_group_by" do
    context "when group_by_field is not present" do
      it "returns false" do
        expect(Reports::TimeEntries::GroupBy.new(nil).is_valid_group_by?).to eq(false)
      end
    end

    context "when invalid group_by_field is passed" do
      it "returns false" do
        expect(Reports::TimeEntries::GroupBy.new("invalid_option").is_valid_group_by?).to eq(false)
      end
    end
    ...
    context "when group_by_field is `team_member`" do
      ...
      it "returns false" do
        expect(Reports::TimeEntries::GroupBy.new(group_by).is_valid_group_by?).to eq(true)
      end
    end
