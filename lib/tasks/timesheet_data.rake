    ...
    if ENV["SEED_DATA_FROM_CSV"].present?
      csv_files.each do |csv_file|
        CSV.foreach(csv_file, headers: true) do |row|
          ...
          client_email = "#{client_name.downcase.gsub(' ', '_')}@client.com"
          ...
          client = Client.find_or_create_by(name: client_name, company_id: company.id, email: client_email)
        ...
        puts "Seed data import complete for #{csv_file}."
      end
    end
  end
end
