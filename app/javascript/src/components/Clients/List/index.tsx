...
const Clients = ({ isAdminUser }) => {
  ...
  const handleSelectChange = (event) => {
    clientApi.get(`?time_frame=${event.target.value}`).then((res) => {
  ...
  useEffect(() => {
    ...
    setAuthHeaders();
    registerIntercepts();
    clientApi.get("?time_frame=week").then((res) => {
      const sanitized = unmapClientList(res);
      setClientData(sanitized.clientList);
      setTotalMinutes(sanitized.totalMinutes);
      setOverDueOutstandingAmt(sanitized.overdueOutstandingAmount);
  ...
  if (loading) {
    return (
      <p className="flex items-center justify-center min-h-screen text-miru-han-purple-1000 tracking-wide text-base font-medium">
        Loading...
      </p>
    );
  }
