...
import { AlertCircle, TrendingUp, Calendar, DollarSign, Download, ChevronDown, CalendarIcon } from "lucide-react";
...
const AccountsAgingReport: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery<AccountsAgingData>({
    ...
    queryFn: async () => {
      ...
      const response = await axios.get(`/reports/accounts_aging?${params.toString()}`);
  ...
  const downloadMutation = useMutation({
    mutationFn: async (formatType: "csv" | "pdf") => {
      ...
      const response = await axios.get(`/reports/accounts_aging/download?${params.toString()}`, {
        responseType: "blob",
      });
      ...
      link.href = url;
      const filename = `accounts_aging_${format(new Date(), "yyyy-MM-dd")}.${formatType}`;
  ...
  const columns: ColumnDef<AccountAging>[] = [
    ...
    },
    {
      ...
      header: () => <div className="text-right whitespace-nowrap">1-30 Days</div>,
      cell: ({ row }) => {
        const value = row.getValue("1_30_days") as number;
        return (
          <div className={cn("text-right whitespace-nowrap", value > 0 ? "text-amber-600 font-medium" : "text-gray-300")}>
    ...
    },
    {
      ...
      header: () => <div className="text-right whitespace-nowrap">31-60 Days</div>,
      cell: ({ row }) => {
        const value = row.getValue("31_60_days") as number;
        return (
          <div className={cn("text-right whitespace-nowrap", value > 0 ? "text-orange-600 font-medium" : "text-gray-300")}>
            {formatCurrency(value || 0, data?.currency)}
          </div>
        );
      },
    },
    {
      ...
      header: () => <div className="text-right whitespace-nowrap">61-90 Days</div>,
      cell: ({ row }) => {
        const value = row.getValue("61_90_days") as number;
        return (
          <div className={cn("text-right whitespace-nowrap", value > 0 ? "text-rose-600 font-medium" : "text-gray-300")}>
            {formatCurrency(value || 0, data?.currency)}
          </div>
        );
      },
    },
    {
      ...
      header: () => <div className="text-right whitespace-nowrap">90+ Days</div>,
      cell: ({ row }) => {
        const value = row.getValue("over_90_days") as number;
        return (
          <div className={cn("text-right whitespace-nowrap", value > 0 ? "text-red-600 font-bold" : "text-gray-300")}>
            {formatCurrency(value || 0, data?.currency)}
          </div>
        );
      },
  ...
  return (
    <div className="space-y-6 p-4 w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Accounts Aging Report</h1>
          ...
          <Card className="border-gray-200">
            ...
            <CardContent>
              <div className="text-xl font-bold text-amber-600">
                {formatCurrency(data?.summary?.["1_30_days"] || 0, data?.currency)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200">
            ...
            <CardContent>
              <div className="text-xl font-bold text-orange-600">
                {formatCurrency(data?.summary?.["31_60_days"] || 0, data?.currency)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200">
            ...
            <CardContent>
              <div className="text-xl font-bold text-rose-600">
                {formatCurrency(data?.summary?.["61_90_days"] || 0, data?.currency)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200">
            ...
            <CardContent>
              <div className="text-xl font-bold text-red-600">
                {formatCurrency(data?.summary?.["over_90_days"] || 0, data?.currency)}
              </div>
            </CardContent>
          </Card>
        </div>
