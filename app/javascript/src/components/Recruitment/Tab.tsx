...
const Tab = ({ isAdminUser }) => {
  ...
  const defaultClassName =
    "mr-10 text-base tracking-widest font-medium text-miru-han-purple-600";
  const activeClassName =
    "mr-10 text-base tracking-widest font-bold text-miru-han-purple-1000 border-b-2 border-miru-han-purple-1000";
  ...
  const setActiveTabKey = useCallback(async () => {
    const filterPath = ["consultancies", "candidates"].filter(
      (key, _index) => location.pathname.replace(/\/+$/, "") === PATHS[key]
    );
    if (filterPath[0]) {
        ...
        <div className="border-b border-gray-200 dark:border-gray-700">
          <ul className="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
            <li className="mr-3">
              <button
                className={
                  tabClassName ? tabClassName.consultanciesTab : activeClassName
                }
                onClick={() => handleTabChange("consultancies")}
              >
            ...
            <li className="mr-3">
              <button
                className={
                  tabClassName ? tabClassName.candidatesTab : defaultClassName
                }
