...
function getStoredBrowserTimeZone(): string {
  try {
    return localStorage.getItem("miru_browser_timezone") || Intl.DateTimeFormat().resolvedOptions().timeZone || "";
  } catch {
    return "";
  }
}
...
function defaultCurrencyForCountry(countryCode?: string): string {
  return COUNTRY_TO_CURRENCY[(countryCode || getStoredBrowserCountry() || "US").toUpperCase()] || "USD";
}

function defaultDateFormatForCountry(countryCode?: string): string {
  const country = (countryCode || getStoredBrowserCountry() || "US").toUpperCase();
