...
const fetchHolidays = async (year: number): Promise<Holiday[]> => {
  try {
    ...
    if (data.holidays && data.holidays.length > 0) {
      const yearHoliday =
        data.holidays.find((h: any) => h.year === year) || data.holidays[0];
...
export const useCalendarData = (
  selectedDate: Date,
  options?: { includeTimeoff?: boolean; includeTimesheet?: boolean }
) => {
  ...
  holidays.forEach(holiday => {
    ...
    const event = {
      ...
      description: `${
        holiday.holiday_type === "national" ? "National" : "Optional"
      } Holiday`,
      calendarId: "holidays",
      _customContent: {
        type: "holiday",
  ...
  if (includeTimeoff) {
    timeoffEntries.forEach(entry => {
      ...
      const leaveIcon = entry.leave_type_name.toLowerCase().includes("sick")
        ? "🏥"
        : entry.leave_type_name.toLowerCase().includes("vacation")
        ? "🏖️"
