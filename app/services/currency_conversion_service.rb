...
class CurrencyConversionService
  def self.get_exchange_rate(from_currency, to_currency, _date = nil)
    return 1.0 if from_currency.to_s.upcase == to_currency.to_s.upcase

    CurrencyPair.get_rate(from_currency.to_s.upcase, to_currency.to_s.upcase)
  end

  def self.convert(amount, from_currency, to_currency, _date = nil)
    new(amount:, from_currency:, to_currency:).process
  end
