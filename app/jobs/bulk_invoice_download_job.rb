...
class BulkInvoiceDownloadJob < ApplicationJob
  ...
  def perform(invoice_ids, company_logo, download_id, root_url, current_url_options, company_id = nil)
    ...
    bulk_download_status = BulkInvoiceDownloadStatus.find_or_create_by!(download_id:, company_id:) do |status|
      status.status = "processing"
    end
    bulk_download_status.update!(status: "processing", file_url: nil)
