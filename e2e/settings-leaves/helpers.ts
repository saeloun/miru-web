/**
 * Settings / Leaves — E2E test helpers.
...
/** Fetch timeoff entries for a user and year. */
export async function fetchTimeoffEntries(page: Page, userId: number | string, year: number) {
    const res = await page.request.get(`/api/v1/timeoff_entries?user_id=${userId}&year=${year}`);
    expect(res.ok(), `Failed to fetch timeoff entries: ${res.status()}`).toBeTruthy();
    return res.json();
}
...
/** Navigate to the leaves settings page and wait for it to settle. */
export async function goToLeaves(page: Page) {
    await page.goto("/settings/leaves");
    // Wait for the summary cards to render (confirms data loaded)
    await expect(
        page.getByText(/time away used/i),
