...
The initial test implementation used incorrect API endpoints that don't exist in
the Miru application.
...
- Updated `getNotificationPreference()` to use
  `/api/v1/team/${userId}/notification_preferences`
- Updated `updateNotificationPreference()` to use
  `/api/v1/team/${userId}/notification_preferences`
...
- Updated `deleteTimesheetEntriesForWeek()` to use `/api/v1/timesheet_entry/:id`
  (singular)
...
4. **Adjust selectors** in settings UI tests if needed (based on actual UI
   structure)
