/**
 * Settings / Preferences — E2E test helpers.
...
export async function fetchPreferences(page: Page, userId: number | string) {
    const res = await page.request.get(`/api/v1/team/${userId}/notification_preferences`);
    expect(res.ok(), `Failed to fetch preferences: ${res.status()}`).toBeTruthy();
    return res.json();
}

/** Update notification preferences for a user via the API. */
export async function updatePreferences(
    page: Page,
    userId: number | string,
    payload: Record<string, boolean>,
) {
    const res = await page.request.patch(`/api/v1/team/${userId}/notification_preferences`, {
        data: { notification_preference: payload },
    });
    expect(res.ok(), `Failed to update preferences: ${res.status()}`).toBeTruthy();
    return res.json();
}
...
/** Navigate to the preferences settings page and wait for it to settle. */
export async function goToPreferences(page: Page) {
    await page.goto("/settings/preferences");
    // Wait for the page title to render after API fetch
    await expect(
        page.getByText(/email preferences/i).first(),
